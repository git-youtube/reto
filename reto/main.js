...
if (localStorage.getItem("token")) {
    ...
    boton.innerHTML = "<button id='logoutBtn'>Cerrar Sesion</button>";
    //Cerrar sesión
...
formulario.addEventListener("submit", function (event) {
    ...
    fetch('http://192.168.1.43/api/login', {
        method: 'POST',
        headers: {
...
formularioR.addEventListener("submit", function (event) {
    ...
    fetch('http://192.168.1.43/api/register', options)
        .then(response => response.json())
...
async function getUsers() {
    ...
    await fetch('http://192.168.1.43/api/empresas', {
      method: 'GET',
      headers: {
...
async function getCotizacion(nombre) {
    ...
    await fetch('http://192.168.1.43/api/ultima/' + nombre, {
        method: 'GET',
        headers: {
...
async function getDiferencia(nombre) {
    ...
    await fetch('http://192.168.1.43/api/resto/' + nombre, {
        method: 'GET',
        headers: {
            // Agrega la autorización y el tipo de contenido en los headers
            'Authorization': 'Bearer ' + localStorage.getItem('token'),
...
async function getCotizacionEmpresa(nombre) {
    await fetch('http://192.168.1.43/api/datos/' + nombre, {
        method: 'GET',
        headers: {
            'Authorization': 'Bearer ' + localStorage.getItem('token'),
            'Content-Type': 'application/json'
